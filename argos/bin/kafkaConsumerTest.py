...
consumer = KafkaConsumer(args.topic,
                         ...
                         group_id='my-group',
                         value_deserializer=toPandasDeserializer
                         )
...
df = pandas.DataFrame(columns=['u', 'v', 'w', 'T'])
lastTime = None
...
if window % slide != 0:
    raise ValueError('window value must be divided by the slide value')

n = int(window/slide)

for message in consumer:
    # print('----------',pandas.Timestamp.fromtimestamp(message.timestamp/1000.0),'----------')
    df = df.append(message.value, sort=True)
    if lastTime is None or lastTime+pandas.Timedelta('%ss' % slide)<message.value.index[0]:
    ...
    if len(timeList)>n:
        try:
            if lastTime!=timeList[0]:
                lastTime = timeList[0]
                print(resampled_df.get_group(timeList[0]))
                # import pdb
                # pdb.set_trace()
                df = df[timeList[1]:]
