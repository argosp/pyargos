...
class DeviceControllerApi(object):
    ...
    def getTenantDevices(self, limit, **kwargs):
        all_params = ['type','text_search','id_offest','text_offset','limit']
        all_params.append('async')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
        ...
        #for field in ['type','text_search','id_offest','text_offset','limit']:
        #    if field in params:
        #        query_params.append((field, params[field]))

        [query_params.append((field, params[field])) for field in all_params if field in params]
        ...
        #{?type,textSearch,idOffset,textOffset,limit}
        dd = self.api_client.call_api('/api/tenant/devices', 'GET',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
